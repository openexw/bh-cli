...
```
bh-cli weight latest [--format <format>]
```
...
bh-cli weight latest --format pretty
```
...
```bash
# 查询最近 7 天
bh-cli weight list --start-date 2026-04-08 --end-date 2026-04-15

# 查询全部，每页 50 条
bh-cli weight list --page-size 50

# 表格格式查看
bh-cli weight list --start-date 2026-04-01 --format table
```
...
```
bh-cli weight create --weight <kg> --measure-time <datetime> [flags]
```
...
END=$(date +%Y-%m-%d)
bh-cli weight list --start-date "$START" --end-date "$END" --format json \
  | jq '.items[] | {date: .measure_time, weight: .weight}'
```

**获取最新体重并分析**：
```bash
bh-cli weight latest --format json | jq '{
  weight,
  bmi,
  body_fat,
...
**批量导出为 CSV**：
```bash
bh-cli weight list --page-size 100 --format csv > weight_export.csv
```
